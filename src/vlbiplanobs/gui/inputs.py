    ...
    return html.Div([dbc.Card([dbc.CardImg(src=app.get_asset_url(f"network-{network.lower()}.png"),
                    ...
                    dbc.CardImgOverlay(dbc.CardBody([
                        html.Div(className='d-flex align-items-center',
                                 children=[
                                    html.H4(network,
                                            className='text-bold text-white mb-0 mt-0 flex-grow-1'),
...
def antenna_list(app, antennas) -> list:
    ...
    """
    return [card(dbc.Accordion(start_collapsed=True, className='accordion-contents-open-ids', flush=True,
                               persistence=True, id='accordion-state',
                               children=[dbc.AccordionItem(title=html.H4(
                                   ['Manual Selection of Antennas   ',
                                    html.I(className='fa fa-solid fa-angle-down', id='accordion-ant')],
                                   className='text-dark font-weight-bold mb-0 accordion-header'),
                                                           children=[
                                    dmc.MantineProvider(dmc.Group(
